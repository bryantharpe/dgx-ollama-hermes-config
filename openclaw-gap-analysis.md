...
1. **Close the model-identity question first (A4).** Decide in one sitting whether the assistant should run on base Qwen3.6-35B-A3B or on an actual Hermes fine-tune, and either change `openclaw.json.agents.defaults.model.primary` to the Hermes tag or rename the project narrative. Everything downstream (tool-call reliability expectations, fallback routing, system prompt) depends on this.

2. **Turn on vector memory (A1).** Pull an embedding model (`ollama pull bge-m3` or `nomic-embed-text`), wire `memory-lancedb` to `http://ollama:11434/v1`, flip `autoCapture` + `autoRecall`. That alone moves memory from "searchable journal" to "actually recalls context". No external creds required.

3. **Pick one external channel and wire it end-to-end (A3), together with A6's role split.** Slack or Telegram — whichever you'd actually answer from. Add the bundled skill to `allowBundled`, create a scoped `responder` role for that device, and populate `HEARTBEAT.md` (B2) with 2–3 concrete checks so the first heartbeats after pairing do something useful. That single change flips the assistant from "local web app" to "in my pocket".
...
- **Paired devices:** 3 (all with full `operator.admin` scope).
